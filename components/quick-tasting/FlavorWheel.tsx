...
import { FlavorCategory, flavorProfiles, hexToRgb, createWheelSegment } from './flavorWheelData';
...
const Flavatix: React.FC<FlavatixProps> = React.memo(
  ({ category, selectedFlavors, onFlavorSelect }) => {
    ...
    const getCircleStyle = (currentScore: number, buttonLevel: number, categoryName: string) => {
      ...
      if (isSelected) {
        ...
        return {
          background: `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${baseOpacity})`,
                      ...
                      <g key={cat.name}>
                        ...
                        <path
                         ...
                         `}
                          style={{
                            filter: isHovered
                              ? 'url(#glow) brightness(1.1) saturate(1.2) url(#innerShadow)'
                              : isSelected
                                ? 'brightness(1.15) saturate(1.1) url(#innerShadow)'
                                : 'url(#innerShadow)',
                      ...
                      <div className="flex gap-xs justify-center">
                        {[1, 2, 3, 4, 5].map((score) => (
                          <button
                            key={score}
                            onClick={() =>
                              updateFlavorScore(flavor, currentScore === score ? 0 : score)
                            }
                            ...
                            onMouseEnter={(e) => {
                              ...
                              if (currentScore < score) {
                                ...
                                e.currentTarget.style.background = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.2)`;
                            ...
                            onMouseLeave={(e) => {
                              const originalStyle = getCircleStyle(
                                currentScore,
                                score,
                                activeCategory
                              );
  ...
  (prevProps, nextProps) => {
    // Only re-render if category or selectedFlavors actually change
    return (
