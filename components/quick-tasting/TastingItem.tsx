...
const TastingItem: React.FC<TastingItemProps> = React.memo(
  ({
    item,
    category,
    userId,
    ...
    return (
      <div
        className="card-premium p-4 sm:p-6 mobile-container mobile-touch animate-fade-in"
      ...
      >
        ...
        <div className="flex flex-col tablet:flex-row tablet:items-center tablet:justify-between gap-sm mb-md">
          <div className="flex items-center space-x-sm min-w-0 flex-1">
            <div className="flex items-center justify-center w-10 h-10 tablet:w-12 tablet:h-12 flex-shrink-0">
              {getCategoryIcon(category)}
            </div>
            <div className="min-w-0 flex-1">
              {/* Item # Label */}
              {!isBlindItems && itemIndex && (
                <div className="text-xs font-medium text-fg-subtle dark:text-fg-subtle mb-1">
                  Item #{itemIndex}
                </div>
              )}
              {isEditingName ? (
                  ...
                  <div key={index} className="mb-5">
                    ...
                    {category.hasText && (
                      <textarea
                        ...
                        onChange={(e) =>
                          handleStudyCategoryChange(category.name, {
  ...
  (prevProps, nextProps) => {
    // Only re-render if item data or key props actually change
    return (
      ...
      prevProps.item.aroma === nextProps.item.aroma &&
      prevProps.item.flavor === nextProps.item.flavor &&
      prevProps.item.photo_url === nextProps.item.photo_url &&
      JSON.stringify(prevProps.item.flavor_scores) ===
        JSON.stringify(nextProps.item.flavor_scores) &&
      prevProps.category === nextProps.category &&
      prevProps.isBlindItems === nextProps.isBlindItems &&
      prevProps.isBlindAttributes === nextProps.isBlindAttributes
    );
  }
);

TastingItem.displayName = 'TastingItem';
