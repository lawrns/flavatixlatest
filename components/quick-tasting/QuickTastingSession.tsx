...
const QuickTastingSession: React.FC<QuickTastingSessionProps> = React.memo(
  ({ session, userId, onSessionComplete, onSessionUpdate, onSessionCreate: _onSessionCreate }) => {
    ...
    const [phase, setPhase] = useState<'setup' | 'tasting'>(
    ...
    const loadTastingItems = async () => {
      ...
      try {
        logger.debug('Tasting', `Loading items for session`, {
          sessionId: session.id,
          mode: session.mode,
          phase,
          isLoading,
    ...
    const deleteLastItem = async () => {
      ...
      const hasCustomName =
        !hasDefaultName && lastItem.item_name && lastItem.item_name.trim() !== '';
      ...
      try {
        const { error } = await supabase.from('quick_tasting_items').delete().eq('id', lastItem.id);
    ...
    const handleCategoryChange = async (newCategory: string) => {
      if (!session || newCategory === session.category) {
        return;
      } // No change needed
    ...
    const completeSession = async () => {
      ...
      if (emptyItems.length > 0) {
        toast.error(
          `Please score all items before completing. ${emptyItems.length} item(s) missing scores.`
        );
        return;
      }
      ...
      items.forEach((item, index) => {
        logger.debug(
          `  ${index + 1}. ${item.item_name} (ID: ${item.id}, Score: ${item.overall_score})`
        );
      });
      ...
      if (session.mode === 'study' && session.notes) {
        try {
          ...
          if (studyMetadata.categories && Array.isArray(studyMetadata.categories)) {
            logger.debug(
              'Study Mode',
              `Loaded ${studyMetadata.categories.length} categories from metadata`
            );
    ...
    useEffect(() => {
      ...
      if (autoAddTriggeredRef.current || hasAutoAddedRef.current) {
        return;
      }
    ...
    return (
      <div className="max-w-6xl mx-auto" data-testid="quick-tasting-session">
        ...
        {showEditTastingDashboard && phase === 'setup' && (
          <div className="mb-lg">
            <EditTastingDashboard session={session} onSessionUpdate={onSessionUpdate} />
          </div>
        )}
            ...
            <div className="space-y-6">
              ...
              {currentItem ? (
                <TastingItem
                  ...
                  onUpdate={(updates: Partial<TastingItemData>) =>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {items.map((item, index) => (
                    ...
                    >
                      ...
                      {item.overall_score && (
                        <div
                          className={`text-xs mt-1 ${index === currentItemIndex ? 'text-white/80' : 'text-fg-subtle'}`}
                        >
  ...
  (prevProps, nextProps) => {
    ...
    // Only re-render if session ID changes or critical session data changes
    return (
...
QuickTastingSession.displayName = 'QuickTastingSession';
