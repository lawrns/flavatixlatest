...
export default function ShareButton({ disabled, onShare, className = '' }: ShareButtonProps) {
  ...
  const [shareData, setShareData] = useState<{ imageUrl?: string; text: string; url: string } | null>(null);
      ...
      {showModal && shareData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-xl max-w-md w-full p-6 relative">
            ...
            {/* Share Text */}
            <p className="text-gray-700 mb-4 text-sm bg-gray-50 p-3 rounded-md">
              {shareData.text}
            </p>
