...
export default function MyTastingsPage() {
  ...
  const loadTastings = useCallback(async () => {
    ...
    try {
      ...
      setTastings(filteredData.slice(0, ITEMS_PER_PAGE));
    } catch (error) {
      console.error('Error loading tastings:', error);
      toast.error('Failed to load tastings');
    } finally {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Delete ${idsToDelete.length} tastings? This action cannot be undone.`)) return;

    const supabase = getSupabaseClient();

    try {
      for (const id of idsToDelete) {
        await supabase.from('quick_tastings').delete().eq('id', id).eq('user_id', user!.id);
      }
